...
function [model] = train(config)
% Inputs:
%   config: A struct containing the following fields
%           <> epochs (default 1): how many sweeps should happen through the training dataset.
%           <> batchsize (default 100): how many training instances should each batch have.
%           <> learning_rate (default 0.1): TODO  
%           <> momentum (0.9): The momentum coeficient used in the momentum method that calculates the deltas for the weights.
%           <> numhid1 (default 50): How many (linear) units exist in the first hidden layer.
%           <> numhid2 (default 200): How many (logistic) units exist in the second hidden layer.
%           <> init_wt (default 0.01): 
%                 (original explanation: Standard deviation of the normal distribution which is sampled to get the initial weights)
%                 (my explanation: The maximum value that a weight could have, before training)
%           <> show_training_CE_after (default 100): 
%                 the number of iterations between each print of the training error.
%           <> show_validation_CE_after (default 1000): 
%                 the number of iterations between each calculation, and print, of the validation error.
% Output:
%   model: A struct containing the following fields
%           <> several fields with the learned weights, biases and vocabulary: 
%                 word_embedding_weights, embed_to_hid_weights, hid_to_output_weights, 
%                 hid_bias, output_bias, vocab
%           <> stats: a struct with the stats of the batches and epochs.
%           <> config: the configuration used for the training process.
% A struct containing the learned weights and biases and vocabulary.
...
if (isfield(config, 'numhid1')) 
  numhid1 = config.numhid1;
end

if (isfield(config, 'numhid2')) 
  numhid2 = config.numhid2;
end

if (isfield(config, 'init_wt')) 
  init_wt = config.init_wt;
end

if (isfield(config, 'show_training_CE_after')) 
  show_training_CE_after = config.show_training_CE_after;
end

if (isfield(config, 'show_validation_CE_after')) 
  show_validation_CE_after = config.show_validation_CE_after;
end

model.config = {};
model.config.epochs = epochs;
model.config.batchsize = batchsize;
model.config.learning_rate = learning_rate;
model.config.momentum = momentum;
model.config.numhid1 = numhid1;
model.config.numhid2 = numhid2;
model.config.init_wt = init_wt;
model.config.show_training_CE_after = show_training_CE_after;
model.config.show_validation_CE_after = show_validation_CE_after;
...
for epoch = 1:epochs
  ...
  this_chunk_CE = 0;
  trainset_CE = 0;
  model.stats.epochs(epoch).batchs_train_ce = [];
  model.stats.epochs(epoch).batchs_train_ce_i = [];
  model.stats.epochs(epoch).batchs_validation_ce = [];
  model.stats.epochs(epoch).batchs_validation_ce_i = [];
  %batchcount = 1;
  ...
  for m = 1:numbatches
    ...
    if mod(m, show_training_CE_after) == 0
      model.stats.epochs(epoch).batchs_train_ce_i(end+1) = m;
      model.stats.epochs(epoch).batchs_train_ce(end+1) = this_chunk_CE;
      fprintf(1, '\n');
      count = 0;
      this_chunk_CE = 0;
    end
    ...
    if mod(m, show_validation_CE_after) == 0
      ...
      model.stats.epochs(epoch).batchs_validation_ce_i(end+1) = m;
      model.stats.epochs(epoch).batchs_validation_ce(end+1) = CE;
      if OctaveMode
        fflush(1);
      end
    end
  ...
  model.stats.epochs(epoch).avg_train_ce = trainset_CE;
end
...
model.stats.final_train_ce = trainset_CE;
...
model.stats.final_validation_ce = CE;
if OctaveMode
  fflush(1);
end
...
model.stats.training_time_seconds = diff;
end
